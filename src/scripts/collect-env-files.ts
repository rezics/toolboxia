...
const collectEnvFilesCommand = defineCommand({
  meta: {
    ...
    description:
      'Recursively collect .env files from the project into a separate directory',
  },
  args: {
    target: {
      type: 'positional',
  ...
  run({args}) {
    ...
    const collectedCount = collectEnvFiles(
      resolvedTarget,
      resolvedTarget,
      outputDir,
    );
    ...
    if (collectedCount === 0) {
      ...
      console.log(
        `Done. Collected ${collectedCount} env files to ${outputDir}`,
      );
    }
  },
});
